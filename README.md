...
- **Архив задач**: закрытие с отметкой выполнена/не выполнена, страница `/projects/:projectId/archive`, восстановление на доску.
- **Публичная read-only доска**: `/public/:shareId` без логина (включается владельцем на странице проекта).
- **Workspace**: компания, мессенджер, поддержка и расширенный профиль — данные на сервере (`/workspace/{key}`).
...
| GET/PUT | `/workspace/{store_key}` | Данные workspace пользователя (`company`, `messenger`, `support`, `profile`) |
